...
#TODO include normal preference distribution and revenue redistribution on the validation procedure
...
class aamas20(experiment):
    ...
    # AAMAS-20 algorithm is here called indifferent_MCT
    __algs_list = ['aamas17', 'aamas17stdql', 'ala18', 'deltatolling', 'differencerewards', 'indifferentMCT', 'stdql', 'trc18', 'weightedMCT']
    ...
    def run_batch_file(self, params):
        ...
        revenue_division_rate = params['revenue-division-rate']
        rep = params['rep']
        flex_dist_name = params['flex-dist']
        flex_dist_params = params['flex-dist-params']
        avf = params['avf']
        ...
        for it in xrange(1, rep+1):
            ...
            # define the distribution of time flexibilities
            flex_dist = Distribution(dist=Distribution.get_dist_id(flex_dist_name), num_of_samples=P.get_total_flow(), params_as_list=flex_dist_params)
            ...
            try:
                ...
                elif alg == 'indifferentMCT':
                    A_POSTERIORI_MCT = True
                    INDIFFERENT_MCT = True
                elif alg == 'weightedMCT':
                    A_POSTERIORI_MCT = True
                    WEIGHTED_MCT = True
    ...
    def validate_script(self):
        ...
        algs_to_ignore = ['stdql', 'weightedMCT']
    ...
    def add_subparser_arguments(subparsers):
        ...
        subp = subparsers.add_parser('aamas20', help='experiments of the AAMAS-20 paper')

        subp.add_argument('--alg', dest='alg', action='store', default='indifferent_MCT', type=str, choices=aamas20.__algs_list,
                            help='algorithm to use (GTQ-learning, which was introduced in the AAMAS-20 paper, is referred to here as indifferent_MCT')
        ...
        subp.add_argument('--flex-dist', dest='flex-dist', action='store', default='DIST_FIXED', type=str, choices=sorted(Distribution.get_list_of_distributions()),
                            help='distribution of drivers\' time flexibility')
        subp.add_argument('--flex-dist-params', dest='flex-dist-params', nargs='+', action='store', type=float, required=False,
                            help='parameters of the chosen distribution; the set of parameters varies depending on the distribution (see documentation for more details)')
        subp.add_argument('--avf', dest='avf', action='store', default=1.0, type=float,
                            help='number of vehicles controlled by each agent')
        subp.add_argument('--use-avf-dr', dest='use-avf-dr', action='store_false', 
                            help='define that each agent should be considered as \'--avf\' flow units (rather than just one) when computing the difference rewards')
